...
error_reporting = path + '\DatabaseErrors.ini'
databaseinfo = []
WRITE = True
...
def DatabaseConnection():
    ...
    try:
        ...
        errorlog = open(error_reporting, 'a')
    ...
    try:
        Database = MySQLdb.connect(host=Host, user=Username, passwd=Password, db=DatabaseName,
    ...
    try:
        ...
        if row['FeeSched'] != '' and row['FeeSched'] != 0:
            try:
                ...
                errorlog = open(error_reporting, 'a')
                errorlog.write('\n Patient Fee Schedule Lookup Error: ' + str(ex) + ' on line ' + format(
        ...
        if row['ImageFolder']:
            ...
            try:
                ...
                cur.execute("SELECT * FROM document WHERE PatNum = '" + str(PatNum) + "' AND DocCategory = " + str(
                    PatData['PatientPicturesCategory']))
                define = cur.fetchone()
                if define:
        ...
        if row['ResponsParty'] != '' and row['ResponsParty'] != 0:
            try:
                cur.execute(
                    "SELECT Address, Address2, City, State, Zip, FName, LName, MiddleI, WorkPhone, HomePhone, WirelessPhone, Email FROM patient WHERE PatNum = " + str(
                        row['ResponsParty']))
        ...
        if row['SuperFamily'] != '' and row['SuperFamily'] != 0 and row['SuperFamily'] != PatNum:
            try:
                ...
                if guarantor['MiddleI']:
                    PatData['GuarantorName'] = guarantor['FName'] + " " + guarantor['MiddleI'] + " " + guarantor[
                        'LName']
                else:
        ...
        try:
            ...
            for CompletedProcs in CompletedProcs:
                ...
                if PatData['ProcedureCodeNumber'] != False:
                    cur.execute(
                        "SELECT ProcCode, Descript, AbbrDesc, LaymanTerm, ProcCat FROM procedurecode WHERE CodeNum = " + str(
                            CompletedProcs['CodeNum']))
                    Code = cur.fetchone()
                    if Code['ProcCode']:
            ...
            for Appointment in Appointments:
                ...
                if Appointment['InsPlan1'] != '' and Appointment['InsPlan1'] != 0:
                    try:
                        ...
                        errorlog = open(error_reporting, 'a')
                        errorlog.write('\n Insurance 1 Sql Query Error: ' + str(ex) + ' on line ' + format(
                            sys.exc_info()[-1].tb_lineno))
                        errorlog.close()
                ...
                if Appointment['InsPlan2'] != '' and Appointment['InsPlan2'] != 0:
                    try:
                        ...
                        errorlog = open(error_reporting, 'a')
                        errorlog.write('\n Insurance 2 Sql Query Error: ' + str(ex) + ' on line ' + format(
                            sys.exc_info()[-1].tb_lineno))
                        errorlog.close()
...
def BenefitPercentageLookup(Type, Insurance):
    Percentages = []
    try:
        cur.execute("SELECT * FROM benefit WHERE PlanNum =" + str(Insurance) + " AND CovCatNum=" + str(
            Type) + " AND BenefitType=1")
...
def PrimaryPlanLookup(PatNum):
    if PatNum != "":
        try:
            ...
            errorlog = open(error_reporting, 'a')
            errorlog.write('\n Patient Insurance Plan Lookup Error: ' + str(ex) + ' on line ' + format(
                sys.exc_info()[-1].tb_lineno))
            errorlog.close()
            return False
    else:
...
def CreateUser(number=9223372036854775807, name="", group=1, employee=0, clinic=0, provider=0, hidden=0, popups=0,
               password=1, restricted=0, tasklist=0):
    global WRITE
    if WRITE:
        ...
        try:
            cur.execute("INSERT INTO userod "
                        ...
                        " VALUES ('%i','%s','%i','%i','%i','%i','%i','%i','%i','%i','%i')" % (
                            number, name, group, employee, clinic, provider, hidden, popups, password, restricted,
                            tasklist))
            return True
        except Exception as ex:
            errorlog = open(error_reporting, 'a')
...
def patientnamesearch(searchstring):
    ...
    if num_there(searchstring):
        ...
        if totaldigits >= 6 and totaldigits <= 10:
            ...
            cur.execute(
                "SELECT PatNum, FName FROM patient WHERE HmPhone LIKE '%" + phoneformat + "%' OR WkPhone LIKE '%" + phoneformat + "%' OR WirelessPhone LIKE '%" + phoneformat + "%'")
            output.extend(cur.fetchall())
    ...
    if len(searchstring) >= 1:
        cur.execute(
            "SELECT PatNum, FName FROM patient WHERE LName LIKE '%" + searchstring + "%' OR FName LIKE '%" + searchstring + "%' ORDER BY PatNum LIMIT 30")
        output = cur.fetchall()
        if len(output) > 0:
            return output
        else:
